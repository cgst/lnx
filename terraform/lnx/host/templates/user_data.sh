...
log_stream_name = ${cluster_name}/{container_ip}
...
container_ip=$(curl 169.254.169.254/latest/meta-data/local-ipv4)
sed -i -e "s/{container_ip}/$container_ip/g" /etc/awslogs/awslogs.conf
